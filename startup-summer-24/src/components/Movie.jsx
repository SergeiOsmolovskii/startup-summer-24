...
export const Movie = ({ movie, allGenres }) => {

  const poster = movie.backdrop_path ? `${IMAGES_BASE_URL}${movie.backdrop_path}` : noPoster;
  /* Multiply by 1000 because of the value from the API */
  const popularity = formatPopularity(movie.popularity * 1000);
      ...
      <Group align="stretch" justify="space-between" wrap="no-wrap">
        ...
        <Stack align="flex-start" flex="1 1 auto" justify="space-between">
          <Stack align="flex-start" justify="space-between" gap={5}>
            <Text component="h2" fz={20} ta="left" c="var(--purple-500)" fw={600}>{movie.original_title}</Text>
